...
def extract_recommended_ids(text: str) -> list:
    """
    Извлекает ID товаров из тега [RECOMMEND: ID1, ID2] в ответе Claude.
    Возвращает список ID или пустой список если тега нет.
    """
    import re
    match = re.search(r'\[RECOMMEND:\s*([^\]]+)\]', text)
    if not match:
        return []
    return [i.strip() for i in match.group(1).split(',') if i.strip()]


def strip_recommend_tag(text: str) -> str:
    """Убирает тег [RECOMMEND: ...] из текста перед отправкой пользователю."""
    import re
    return re.sub(r'\[RECOMMEND:[^\]]*\]', '', text).strip()
